        ...
        for entity in entities:
            ...
            if thumbnail_path:
                ...
                url = generate_sas_url(
                    container_name, blob_name, account_name, account_key
                )
            ...
            results.append(
                {"id": idx, "url": url, "tags": tags, "created_at": timestamp}
            )
...
def get_images(req: func.HttpRequest) -> func.HttpResponse:
    try:
        ...
        if not idx:
            return func.HttpResponse("No image id in route params", status_code=400)
        connection_string = os.environ["ENV_PHOTOS_CONNSTR"]
        metadata_storage_name = os.environ["ENV_METADATA_STORAGE_NAME"]
        container_name = os.environ["ENV_PHOTOS_CONTAINER_NAME"]
        ...
        if not entity:
            return func.HttpResponse(f"Image with id {idx} not found", status_code=400)
        ...
        result = {"id": idx, "url": url, "tags": tags, "created_at": timestamp}
